  ...
  describe 'defining schema' do
    ...
    describe '#call' do
      ...
      it 'validates presence of the email key and type of age value' do
        attributes = {
          name: 'Jane', age: '18', address: attrs[:address], phone_numbers: attrs[:phone_numbers]
        }

        expect(validation.(attributes)).to match_array([
        ...
        expect(validation.(email: 'jane@doe.org', age: 19)).to match_array([
          ...
          [:error, [
            :input, [
              :phone_numbers, nil,
              [
                [:key, [:phone_numbers, [:predicate, [:key?, [:phone_numbers]]]]]
              ]
            ]
          ]]
        ])
      ...
      it 'validates address type' do
        expect(validation.(attrs.merge(address: 'totally not a hash'))).to match_array([
          [:error, [
            :input, [
              :address, 'totally not a hash',
              [
                [
                  :val, [
                    :address, [:predicate, [:hash?, []]]
                  ]
                ]
              ]
            ]
      ...
      it 'validates address code and name values' do
        address_without_country_name = attrs[:address].merge(country: { code: 'US', name: '' })

        expect(validation.(attrs.merge(address: address_without_country_name))).to match_array([
          [:error, [
            :input, [
          ...
          [:error, [
            :input, [
              ...
              [
                [
                  :input, [
                    :phone_numbers, 312, [[:val, [:phone_numbers, [:predicate, [:str?, []]]]]]
                  ]
                ]
              ]
            ]
